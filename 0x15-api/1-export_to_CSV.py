...
#             user = requests.get(
            # f"{REST_API}users/{user_id}", timeout=5).json()
...
#                     list_format = f"\"{user_id}\",\"{user['name']}\",\
            # "{user_tasks['completed']}\",\"{user_tasks['title']}\""
#                     task_list.append(list_format)
#                     c_file.write(task_list)
